...
class TrajSimi:
    def __init__(self):
        ...
        self.trajsimiregression = TrajSimiRegression(Config.seq_embedding_dim)
        self.trajsimiregression.to(Config.device)
        # self.embedding =np.load("../data/porto_t2vec_hausdorff.npy")
        self.dic_datasets = TrajSimi.load_trajsimi_dataset()

        self.checkpoint_filepath = '{}/{}_trajsimi_{}_best{}.pt' \
            .format(Config.checkpoint_dir, "t2vec", \
    ...
    def train(self):
        ...
        #         optimizer.zero_grad()
        #
        #         trajs, sub_simi = batch         # (L, B, 256)
        #         outs = self.trajsimiregression(trajs)
        #
        ...
        #     hr_eval_ep = (eval_metrics[1] + eval_metrics[2] + eval_metrics[3]) / 3
        ...
        # self.encoder.context_encoder.load_state_dict(checkpoint['encoder_q'])
    ...
    def test(self, dataset_type):
        ...
        # hr_1 = TrajSimi.hitting_ratio(pred_l1_simi, truth_l1_simi, 1, 1)
    ...
    def trajsimi_dataset_generator_single_batchi(self, datasets):
        ...
        while cur_index < len_datasets:
            ...
            trajs = [torch.tensor(traj, dtype=torch.float32) for traj in trajs]
            trajs = torch.stack(trajs, 1).to(Config.device)
            # trajs = pad_sequence(trajs, batch_first=True).to(Config.device)
            ...
            # trajs_emb_cell = pad_sequence(trajs_emb_cell, batch_first=True).to(
    ...
    def trajsimi_dataset_generator_pairs_batchi(self):
        ...
        while count_i < counts:
            ...
            trajs = [torch.tensor(traj, dtype=torch.float32) for traj in trajs]
            trajs = torch.stack(trajs, 1).to(Config.device)
            # trajs = pad_sequence(trajs, batch_first=False).to(Config.device)
            # print(trajs.shape)
            # def convert_trajectory(trajectory, cellspace):
            #     """Convert a trajectory to a list of (cell_id, offset_x, offset_y)."""
            #     return [get_offset(x, y, cellspace) for x, y in trajectory]
    ...
    def hitting_ratio(preds: torch.Tensor, truths: torch.Tensor, pred_topk: int, truth_topk: int):
        ...
        preds = preds + torch.eye(preds.shape[0], device=preds.device) * preds.max()
    ...
    def load_trajsimi_dataset():
        ...
        trajsimi_traj_dataset_file = "/mnt/data728/lihuan/submission/data/porto_t2vec_hausdorff.npy"
        ...
        # print(trains_traj)
        # print(trains_traj.iloc[:10])
        def split(trajsimi_traj_dataset_file):
            data = np.load(trajsimi_traj_dataset_file)
            l = data.shape[1]
            # print(data[:10, :])
            train_idx = (int(l * 0), int(l * 0.7))
            eval_idx = (int(l * 0.7), int(l * 0.8))
            test_idx = (int(l * 0.8), int(l * 1.0))
            train, eval, test = data[:, train_idx[0] : train_idx[1], :], data[:, eval_idx[0] : eval_idx[1], :], data[:, test_idx[0] : test_idx[1], :]
            # print(train.shape)
            return train, eval, test

        trains_traj, evals_traj, tests_traj = split(trajsimi_traj_dataset_file)
        # print(trains_traj)
