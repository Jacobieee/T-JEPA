...
class CellSpace:
    ...
    def get_point_by_xyidx(self, i_x, i_y):
        # Calculate the center of the cell by adding half of the unit size to the lower corner's coordinates
        x = (i_x * self.x_unit) + self.x_min + (self.x_unit / 2)
        y = (i_y * self.y_unit) + self.y_min + (self.y_unit / 2)

        # Return the GPS points at the center of the cell
        return (x, y)
