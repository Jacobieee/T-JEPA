...
def join_data(f1, f2, f3):
    ...
    df.to_csv('../data/tokyo.csv', index=False)
...
def process_ca(file):
    df = pd.read_csv(file)
    # print(df.head(10))
    grouped = df.groupby('pseudo_session_trajectory_id')
    # print(grouped_df.size())

    def create_polyline(group):
        # Create the trajectory string for each row in the group
        trajectory = group[['Longitude', 'Latitude']].apply(
            lambda row: f"[{row['Longitude']}, {row['Latitude']}]", axis=1).tolist()
        # Join all coordinate pairs in the group into a single string
        trajectory_str = f"[{', '.join(trajectory)}]"
        return trajectory_str

    traj = grouped.apply(create_polyline).reset_index(name='POLYLINE')

    traj.to_csv('../data/tokyo.csv', index=False)
    print(traj)
...
def clean_and_output_data():
    ...
    dfraw = pd.read_csv(Config.root_dir + '/data/tokyo.csv')
    # print(dfraw)
...
if __name__ == '__main__':
    ...
    Config.dataset = 'tokyo'
    Config.post_value_updates()

    # join_data("../tky/train_sample.csv", "../tky/validate_sample_with_traj.csv", "../tky/test_sample_with_traj.csv")
    # get_mbr("../data/tokyo.csv")
    # process_ca("../data/tky_raw.csv")
